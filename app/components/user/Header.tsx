...
import { isConnected, getUserName } from "@/src/db/lib/actions";
...
export default function Header({ openModal, openModalSignOut }: HeaderProps) {
  ...
  useEffect(() => {
    isAdmin().then(setAdmin);
     isConnected().then(setUserConnect);
    getUserName().then(setUserName);  // ← récupère le nom
  }, []);
  ...
  <div className="w-px h-5 bg-blue-100" />
            ...
            {/* Profil — affiché uniquement si connecté et userName connu */}
            {userConnect && userName && (
              <>
                <Link
